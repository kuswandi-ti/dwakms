...
<html lang="en-US"><!--<![endif]-->
	...
	<head>
		...
		<title><?= $MAIN_PAGE_TITLE; ?> | Problem</title>
					...
					<div class="row clearfix">
						...
						<div class="grid_4 sidebar">
							<div id="categories-3" class="widget widget_categories">
								<h3 class="col-title">Problem Category</h3>
								<span class="liner"></span>	
								
								<?php
								
									$res = mysqli_query($conn, "select * from tbl_problem_kategori order by problem_kategori");
									$num_rows = mysqli_num_rows($res);
									if ($num_rows > 0) {
										echo "<ul>";
											while ($data = $res->fetch_array()) {	
												echo '<li>';
												echo '	<a href="list_problem.php?cat_id='.$data['id'].'&cat_name='.$data['problem_kategori'].'">'.$data['problem_kategori'].'</a>';
												echo '</li>';
											}
										echo "</ul>";
									} else {
						...
						<div class="grid_8 sidebar omega">	
							...
							<div id="recent-posts-4" class="widget widget_recent_entries">	
								<h3 class="col-title">List Problem : <?=$_GET['cat_name']?></i></h3>
								<span class="liner"></span>	
								...
								<?php									
									...
									$res = mysqli_query($conn, "select * from tbl_problem
																where problem_kategori_id='".$_GET['cat_id']."'
									...
									if ($num_rows > 0) {
										echo "<ul>";
											while ($data = $res->fetch_array()) {	
												echo '<li>';
												echo '	<a href="problem.php?prb_id='.$data['id'].'">'.$data['topic'].'</a>';
												echo '	<span class="post-date">'.$data['team_leader'].', '.convert_date($data['date_problem']).'</span>';
												echo '</li>';
											}
										echo "</ul>";
									} else {
									...
									$paging = mysqli_query($conn, "select * from tbl_problem
																   where problem_kategori_id='".$_GET['cat_id']."'
																   order by rec_datetime desc");
									...
									for ($i=1;$i<=$jmlhalaman;$i++){
										if ($i != $halaman) {
											echo "<li><a href=\"list_problem.php?halaman=$i\">$i</a></li>";
										} else {
